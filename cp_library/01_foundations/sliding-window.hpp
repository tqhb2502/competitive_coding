...
// Ý nghĩa: đếm số đoạn con (contiguous) có tổng <= K bằng cửa sổ trượt (yêu cầu phần tử không âm).
// Tham số: a = mảng phần tử không âm (0-based); K = ngưỡng tổng tối đa.
// Trả về: tổng số đoạn con hợp lệ (mỗi r cộng thêm số đoạn kết thúc tại r và còn hợp lệ).
...
int main() {
    ...
    printf("số đoạn con có tổng <= 3: %lld\n", countSubarraysSumAtMost(a, 3));   // 4
    return 0;
}
#endif
