...
// CSES: 1159 1644 3221
// Ý nghĩa: min hoặc max của mọi cửa sổ trượt độ dài k trên mảng a, dùng deque đơn điệu chứa index.
// Tham số: a = mảng đầu vào; k = độ dài cửa sổ; wantMin = true lấy min, false lấy max.
// Trả về: vector kết quả n-k+1 phần tử (res[i] = extreme của a[i..i+k-1]); rỗng nếu k<=0 hoặc k>n.
template <class T>
...
// Ý nghĩa: min của mọi cửa sổ trượt độ dài k (bọc windowExtreme với wantMin = true).
// Tham số: a = mảng đầu vào; k = độ dài cửa sổ.
// Trả về: vector n-k+1 phần tử, res[i] = min a[i..i+k-1].
template <class T> vector<T> windowMin(const vector<T>& a, int k) { return windowExtreme(a, k, true); }
// Ý nghĩa: max của mọi cửa sổ trượt độ dài k (bọc windowExtreme với wantMin = false).
// Tham số: a = mảng đầu vào; k = độ dài cửa sổ.
// Trả về: vector n-k+1 phần tử, res[i] = max a[i..i+k-1].
