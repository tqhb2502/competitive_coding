...
class Bundler:
    ...
    def _resolve_include(self, including_file: Path, include_name: str) -> Path:
        ...
        windows_name = PureWindowsPath(include_name)
        # PurePosixPath không xem ``C:/...`` là tuyệt đối. Trên Windows,
        # ``Path("C:/...")`` lại có drive và phép ``directory / path`` sẽ bỏ
        # directory, nên phải chặn cả path absolute/drive-qualified kiểu Windows.
        if (pure_name.is_absolute() or windows_name.is_absolute()
...
def main(arguments: Optional[Sequence[str]] = None) -> int:
    # Snippet có comment tiếng Việt: console Windows mặc định cp1252 sẽ crash UnicodeEncodeError
    # khi in ra stdout (trường hợp không truyền -o). Buộc UTF-8 cho cả stdout và stderr.
    ...
    try:
        ...
        overwrites_input = output_path in bundler.seen
        if output_path.exists() and not overwrites_input:
            # ``resolve()`` bắt symlink nhưng không hợp nhất hardlink: hai tên
            # khác nhau vẫn có thể trỏ tới cùng inode/file ID. Ghi vào alias đó
            # sẽ truncate chính source/header đã đọc.
            overwrites_input = any(
                output_path.samefile(input_path)
                for input_path in bundler.seen
            )
        if overwrites_input:
            raise BundleError(
