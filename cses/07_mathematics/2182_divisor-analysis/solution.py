# Divisor Analysis - CSES 2182
# https://cses.fi/problemset/task/2182
#
# N = tích x_i^{k_i}. Tính (mod 1e9+7):
#   - số ước:    D = tích (k_i + 1)
#   - tổng ước:  S = tích (x_i^{k_i+1} - 1) / (x_i - 1)   (chuỗi hình học)
#   - tích ước:  P = N^{D/2} = tích x_i^{k_i * D / 2}
import sys
...
def main():
    ...
    MOD = 10 ** 9 + 7
    PHI = MOD - 1  # 10^9+6, dùng cho Fermat: x^e = x^(e mod PHI) khi gcd(x, MOD) = 1
    ...
    # (2) Tổng ước: tích (x^{k+1} - 1) * nghịch_đảo(x - 1) mod MOD.
