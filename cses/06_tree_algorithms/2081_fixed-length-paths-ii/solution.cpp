...
int main() {
    ...
    std::vector<char> removed(n + 1, false);
    std::vector<int> parent(n + 1, 0);
    std::vector<int> subtree_size(n + 1, 0);
    std::vector<int> frequency(maximum_length + 1, 0);
    std::vector<int> prefix(maximum_length + 1, 0);
    std::vector<int> component;
    component.reserve(n);
    std::vector<int> component_stack{1};
    long long answer = 0;
    ...
    while (!component_stack.empty()) {
        ...
        // A pair is counted when its second branch is processed. Sorting by
        // branch depth keeps rebuilding the prefix-frequency array linear in
        // the total branch sizes at this centroid.
        std::sort(
        ...
        frequency[0] = 1;  // Pairing with depth zero counts centroid endpoints.
